...
def plt( res_img, cmap, title, fsize, p):
    ...
    image_1 = cv2.imread('imgs.img7.png')
    cvrt_img_1 = cvt.cvtColor(img_1, cv2.COLOR_BGR2RGB)
	...
	#Threshold the HSV image to get only blue colors mask = cv2.inRange (hsv, light_blue, dark_blue)
	#Bitwise-AND mask and original image
	output = cv2.bitwise_and (cvrt_img_1, cvrt_img_1, mask= mask)
	plt.imshow(np.hstack ((cvrt_img_1, output)))
	
	from skimage.io import imshow, imread from skimage.color import rgb2hsv, hsv2rgb
		filter_color_red =(cvrt_img_1[:,:,0] > 150)
		_plt filter_color_red, 'gray', '', '', 2
		
		mask_img = cvrt_img_1.copy()
		=
		mask_img[:,:, 0] = mask_img[:, :, 0]*filter_color_red mask_img[:, :, 1] mask_img[:, :, 1]*filter_color_red mask_img[:, :, 2] = mask_img[:, :, 2]*filter_color_red plt.figure(num=None, figsize=(8, 6), dpi=80)
		_plt mask_img,
		'', 1)
		...
		ax[0].set_title (hsv_list[0], fontsize = 20) ax[0].axis('off')
		=
		ax[1].imshow(img_hsv[:,:,1], cmap = 'Greys') ax[1].set_title (hsv_list[1], fontsize 20) ax[1].axis('off')
		ax[2].imshow(img_hsv[:,:,2], cmap = 'gray') ax[2].set_title (hsv_list[2], fontsize = 20) ax[2].axis('off')
		plt.show()
		
		plt.figure(num=None, figsize=(8, 6), dpi=80) plt.imshow(img_hsv[:,:,0], cmap='hsv')
		plt.colorbar();
	...
	lower_mask_1 = img_hsv[:,:,0] > 95 upper_mask_1 = img_hsv[:,,0] < 155 saturation_1 = img_hsv[:,:,1] > 90
